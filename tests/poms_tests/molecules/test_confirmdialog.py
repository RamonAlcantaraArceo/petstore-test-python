...
def test_confirmdialog_selector_defined():
    ...
    assert ConfirmdialogPOM.SELECTOR, "SELECTOR is empty — run sbpom again with playwright installed"


@pytest.mark.smoke
def test_confirmdialog_story_id_defined():
    assert hasattr(ConfirmdialogPOM, "STORY_ID")
    assert ConfirmdialogPOM.STORY_ID, "STORY_ID is empty — no renderable story found for this component"

@pytest.mark.functional
@pytest.mark.usefixtures("capture_screenshot")
def test_confirmdialog_navigate_and_verify(driver, pom_interaction_helper):
