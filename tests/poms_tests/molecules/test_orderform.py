...
def test_orderform_selector_defined():
    ...
    assert OrderformPOM.SELECTOR, "SELECTOR is empty — run sbpom again with playwright installed"


@pytest.mark.smoke
def test_orderform_story_id_defined():
    assert hasattr(OrderformPOM, "STORY_ID")
    assert OrderformPOM.STORY_ID, "STORY_ID is empty — no renderable story found for this component"

@pytest.mark.functional
@pytest.mark.usefixtures("capture_screenshot")
def test_orderform_navigate_and_verify(driver, pom_interaction_helper):
